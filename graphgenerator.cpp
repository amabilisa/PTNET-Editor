...
{
   QMapIterator<QString, int> p1(t.preSet);
   while (p1.hasNext())
   {
     p1.next();
     int w = p1.value();
     int token = M.value(p1.key());
     if(token < w)
        return false;
   }
...
{
   QMapIterator<QString, int> p2(t.postSet);
   while (p2.hasNext())
   {
     p2.next();
     int w = p2.value();
     int c = places.value(p2.key());
     int token = M.value(p2.key());
...
{
   QMapIterator<QString, int> prePlace(t.preSet);
   while (prePlace.hasNext())
   {
     prePlace.next();
     int w = prePlace.value();
...
{
   QMapIterator<QString, int> postPlace(t.postSet);
   while (postPlace.hasNext())
   {
     postPlace.next();
     int w = postPlace.value();
